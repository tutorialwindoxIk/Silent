...
async def chat_gpt(app, message):
    try:
        ...
        if len(message.command) < 2:
            ...
            await message.reply_text("**Hello sir, I am Lucky. How can I help you today?**")
        else:
...
async def chat_arvis(app, message):
    try:
        ...
        if len(message.command) < 2:
            await message.reply_text(f"**Hello {name}, I am Lucky. How can I help you today?**")
        else:
            query = message.text.split(' ', 1)[1]
            MODEL = "gpt-3.5-turbo"
            resp = openai.ChatCompletion.create(model=MODEL, messages=[{"role": "user", "content": query}],
...
# Command for LUCKY with user's name
@app.on_message(filters.command(["ora"], prefixes=["d", "D"]))
async def chat_lucky(app, message):
    try:
        await app.send_chat_action(message.chat.id, ChatAction.TYPING)
        name = message.from_user.first_name
        if len(message.command) < 2:
